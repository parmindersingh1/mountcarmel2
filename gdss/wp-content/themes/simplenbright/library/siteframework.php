...
function site5framework_queue_js(){ if (!is_admin()){ if ( is_singular() AND comments_open() AND (get_option('thread_comments') == 1)) wp_enqueue_script( 'comment-reply' ); }
}

// reply on comments script
add_action('wp_print_scripts', 'site5framework_queue_js');
...
function site5framework_excerpt_more($more) {
	...
	return '<p class="readmore"> <a href="'. get_permalink($post->ID) . '" title="Read '.get_the_title($post->ID).'">  CONTINUE READING ...</a> </p>';
}

...
function site5framework_header_init() {
	...
	wp_enqueue_style('buttons', get_template_directory_uri().'/lib/shortcodes/css/buttons.css');
...
function site5framework_custom_shortcut_favicon() {
	if (of_get_option('snb_custom_shortcut_favicon') != '') {
    echo '<link rel="shortcut icon" href="'. of_get_option('snb_custom_shortcut_favicon') .'" type="image/ico" />'."\n";
	}
...
function site5framework_portfolio1_init() {
	if (is_page_template('template.portfolio.3cols.php') || is_page_template('template.portfolio.3cols.sidebar.php') || is_post_type_archive(portfolio)  &&  !is_admin()) {
...
function site5framework_analytics(){
	$output = of_get_option('snb_stats');
	if ( $output <> "" )
